...
id=$(ssh -o BatchMode=yes tig-gpu "gpuq submit --project tig-monorepo --commit $sha \
    --branch '$br' --lane $lane --timeout-s 5400 -- env ${BOX_ENV:-} bash scripts/box_test.sh $name $*" 2>/dev/null | tail -1)
echo "job: $id"
rc=0
...
```bash
scripts/box_submit.sh baseline gpu
```
...
`gan_generator/blob.rs`, tests first:
...
Expected: `cannot find function parse_container`. Add `pub mod blob;` is already in `mod.rs` from Step 1.
...
**Interfaces:**
- Produces: `StructuredGate`, `Generator::StructuredGate`, and `pub fn softplus(x: f32) -> f32` (the GPU test in Task 9 does not need it, but the near-threshold count there reuses `gate_margin_cpu`):
  `pub fn gate_margin_cpu(&self, latent: &[f32], gate_noise: &[f32]) -> Vec<f32>` returning `logit_j + smoothed_noise_j` per coordinate.
...
- Produces:
  ...
  // scenarios.rs
  impl Scenario { pub const ALL: [Scenario; N] }     // N grows per task: 2 here, 3 after Task 8
  ```
...
```rust
    ...
    fn all_lists_every_variant_exactly_once() {
        let names: std::collections::BTreeSet<String> = Scenario::ALL.iter().map(|s| s.to_string()).collect();
        assert_eq!(names.len(), Scenario::ALL.len(), "a variant is listed twice");
    }
```
...
`AUDIT_MAX_DIMS` must be reachable from `scenarios.rs`'s tests: change its declaration in `mod.rs` to `pub(crate) const AUDIT_MAX_DIMS: u32 = 128;`. The `lib.rs` agreement test searches for the text `const AUDIT_MAX_DIMS: u32 = `, which `pub(crate) const …` still contains.
...
```rust
    ...
    #[test]
    fn exact_1nn_measures_recall_1_on_glove() {
        let (challenge, module, stream, prop) = gpu_instance_for(Scenario::GLOVE_100, 5);
        let solution = exact_1nn_solution(&challenge, &module, &stream);
        let quality = challenge.evaluate_solution(&solution, &[9u8; 32], module, stream, &prop).unwrap();
        assert_eq!(quality, QUALITY_PRECISION as i32);
    }
```

Also add `all_zeros_measures_recall_near_0_on_glove`: copy the body of `all_zeros_measures_recall_near_0` (line 1056) verbatim with `gpu_instance_for(Scenario::GLOVE_100, ..)`, keeping its bound. That makes 5 GPU tests in this step.

`exact_1nn_solution` and the `evaluate_solution` argument order: copy them from the existing `exact_1nn_measures_recall_1` (line 1044) verbatim; if that test builds its solution inline, lift the inline code into a `fn exact_1nn_solution(challenge, module, stream) -> Solution` helper and make the SIFT test call it too.

Mutations caught: `gpu_forward_matches` → `gan_row_normalize` wrong formula, wrong `out_row_offset` in either the last linear or the normalise, latents read at the wrong stride; `invariant_to_launch_geometry` → any dependence on chunking, including normalising rows `[0, n)` where `[row_offset, row_offset + n)` is meant (chunks after the first would stay un-normalised in one geometry and not another); `unit_norm` → normalise skipped for chunks after the first; `recall_1_on_glove` → the audit mishandling 100 dims (not a multiple of `AUDIT_KC = 16`).
...
`Display`/`FromStr` arms for `"glove_100"`, and:

```rust
impl Scenario {
    /// Every variant. `_exhaustive` has no wildcard arm, so adding a variant
    /// without extending it is a compile error, and the comment there says to
    /// extend this array in the same edit.
    pub const ALL: [Scenario; 2] = [Scenario::SIFT_128, Scenario::GLOVE_100];

    #[allow(dead_code)]
    const fn _exhaustive(s: Scenario) {
        match s {
            // Adding an arm here? Add the variant to ALL above as well.
            Scenario::SIFT_128 | Scenario::GLOVE_100 => {}
        }
    }
}
```
...
Expected: Step 4's count + 3 (scenarios) + 1 (track) + 5 (GPU). Record the wall time of `glove_database_rows_are_unit_norm` as a rough generation-time signal (MEASURED, includes a 280 MB readback).
...
- [ ] **Step 1: Raise all three to 256** (`#define AUDIT_MAX_DIMS 256`, `pub(crate) const AUDIT_MAX_DIMS: u32 = 256;`, `#define REF_MAX_DIMS 256`). Update the shared-memory figure in the kernel comment from 27,720 to 36,936 bytes and say it was computed (`18×256×4 + 256×17×4 + 1,024 + 72`), not read from `ptxas`. Commit, push.
...
In `mod.rs` add `const AUDIT_WIDE_TQ: u32 = 9;` and `pub(crate) const AUDIT_WIDE_MAX_DIMS: u32 = 256;`, and in `measure_recall_with_samples` select by width:
...
```rust
    ...
    #[test]
    fn exact_1nn_measures_recall_1_on_nytimes() {
        let (challenge, module, stream, prop) = gpu_instance_for(Scenario::NYTIMES_256, 5);
        let solution = exact_1nn_solution(&challenge, &module, &stream);
        let quality = challenge.evaluate_solution(&solution, &[9u8; 32], module, stream, &prop).unwrap();
        assert_eq!(quality, QUALITY_PRECISION as i32);
    }
    ...
    #[test]
    fn all_zeros_measures_recall_near_0_on_nytimes() {
        // Copy the body of `all_zeros_measures_recall_near_0` (line 1056) with
        // gpu_instance_for(Scenario::NYTIMES_256, ..); keep its bound.
    }
...
The `all_zeros…_on_nytimes` body above is a copy instruction, not a placeholder: the existing test is 10 lines, and the executor must copy it, not paraphrase it, so the bound stays the one already reviewed.
...
- [ ] **Step 5: Add the scenario** (`scenarios.rs`): variant `NYTIMES_256`, `const NYTIMES_256_BLOB: &[u8] = include_bytes!("weights/nytimes_256_v3.bin");`, arm with `vector_dims: 256` and the same six other values, a comment naming the checkpoint (`v3_best`, `/workspace/nytimes-v3/v3_seed42/best_generator.pt`) and repeating that `min_recall` is unmeasured for this corpus; `Display`/`FromStr` for `"nytimes_256"`; `ALL` becomes `[Scenario; 3]` with the variant added to both `ALL` and `_exhaustive`.

- [ ] **Step 6: Push, run, expect green.** `scripts/box_submit.sh task8-nytimes gpu`. Expected: Task 6's final count + 8: the NYTimes wire test, the NYTimes track test, and the 6 GPU tests above. The dims loop covers the new variant without a new test. Record the wall time of `nytimes_database_rows_are_unit_norm`.
...
Mutations caught: `gpu_forward_matches` → any formula error in `gan_gate_apply`, `coupling` and `smoothing` swapped, `sparsity` indexed per coordinate where per row is meant; zero-fraction → inverted threshold, or `gate_temperature`-style scaling wrongly reintroduced; noise test → the `2^40` base dropped (the noise would then be a deterministic function of the same curand state as the latents; if the correlation assertion does not catch that mutation in Step 6, replace it with the stronger check that `u` recovered from the noise does not equal the uniform that Box-Muller consumed, and say so in the task notes), the upper clamp removed (infinite values appear with probability about 0.4 over 8.4 million draws, so the 700,000-row unit-norm test is the reliable catch: it sees about 5 expected occurrences).
...
First find where each real corpus lives: `ls ~/TIG/wgan-synthetic/data/` here (NYTimes files are present locally: MEASURED) and `ssh tig-gpu 'ls /workspace/checkouts/wgan-synthetic/data/'`. Run wherever the real file is; if on the box, submit through `gpuq --project wgan-synthetic --lane cpu`, and if here, stream the three `.npy` dumps over with the tar-and-hash procedure first. Do **not** pass `--allow-condition-mismatch`: `check_gate` compares the run's N, k and nlist against the conditions pinned in the gate file, and that refusal is the guard against measuring under the wrong conditions.
