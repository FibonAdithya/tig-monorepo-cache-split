...
pub(super) fn take<'a>(blob: &'a [u8], at: &mut usize, len: usize) -> Result<&'a [u8]> {
    let end = at
        .checked_add(len)
...
pub(super) fn read_u32(blob: &[u8], at: &mut usize) -> Result<u32> {
    let b = take(blob, at, 4)?;
    Ok(u32::from_le_bytes([b[0], b[1], b[2], b[3]]))
}

pub(super) fn read_f32s(blob: &[u8], at: &mut usize, count: usize) -> Result<Vec<f32>> {
    let byte_len = count
        .checked_mul(4)
...
pub(crate) mod tests {
    ...
    fn embedded_v1_blob_has_expected_shape() {
        let g = parse_weights(V1_BLOB).unwrap();
    ...
    fn cpu_forward_matches_pytorch_golden_vectors() {
        ...
        let golden: Golden = serde_json::from_str(include_str!(
            "../vector_search/weights/golden_vectors.json"
        ))
        .unwrap();

        let weights = parse_weights(V1_BLOB).unwrap();
